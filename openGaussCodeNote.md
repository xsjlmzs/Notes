...
首先进行词法分析和语法分析，得到解析树。`parsetree_list = pg_parse_query(sql_query_string)`

语义分析后得到查询树，进行重写。`querytree_list = pg_analyze_and_rewrite(parse_tree)`

对已经重写的查询生成执行计划。`plantree_list = pg_plan_queries(querytree_list)`
...
执行器（executor）的输入是优化器产生的计划树(plan tree)，计划树经过执行器转换成执行状态树。
